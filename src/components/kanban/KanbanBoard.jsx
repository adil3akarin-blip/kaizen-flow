...
function KanbanColumnEmpty({ copy }) {
 ...
 <p className="m-0 text-xs leading-snug text-ink-muted">{copy.primary}</p>
 {copy.dragHint && (
 <p className="m-0 mt-1.5 hidden text-[11px] leading-snug text-ink-muted/75 md:block">
 {copy.dragHint}
 </p>
 )}
 </div>
...
function KanbanColumn({ column, cards, dragEnabled, onMoveTap, onEditTap, view }) {
 ...
 <h3 className="m-0 text-sm font-medium text-ink">{column.label}</h3>
 ...
 isAccentedEmpty
 ? 'border-dashed border-accent/25 bg-accent/[0.04]'
...
function KanbanSortableCard({
 ...
 stuck && 'rounded-xl ring-2 ring-warn/60',
...
export default function KanbanBoard({ view }) {
 ...
 card={card}
 compact
 className="scale-[1.02] select-none shadow-lg ring-2 ring-accent/30"
 />
 )
 }}
 </DragOverlay>
