...
import { detectMobilePlatform, isStandalone } from '../../lib/pwaInstall'
        ...
        >
          ...
          >
            ...
            <p className="mt-2 text-sm leading-relaxed text-ink-muted">
              ...
              или пересмотреть затор. На iPhone нужно приложение на экране «Домой».
            </p>
...
export default function NotificationSettings() {
  ...
  const shouldOfferPushPrompt = (enabled) =>
    enabled
    && !pushPromptShown
    && canUseLocalNotifications()
    && Notification.permission === 'default'

  const applyToggle = (id, enabled) => {
    setNotificationEnabled(id, enabled)

    if (shouldOfferPushPrompt(enabled)) {
      setPendingToggle(id)
  ...
  const iosNeedsInstall = detectMobilePlatform() === 'ios' && !isStandalone()
  const remotePushReady = canUseRemotePush()
  const localPushReady = permission === 'granted' && (isIosPwa() || detectMobilePlatform() !== 'ios')
    ...
    >
      ...
      {permission === 'denied' && (
        <p className="mt-3 rounded-lg border border-line/60 bg-sunken/60 px-3 py-2.5 text-xs leading-relaxed text-ink-muted">
          Уведомления заблокированы в системе. Подсказки внутри приложения всё равно
          работают.
        </p>
      )}

      {iosNeedsInstall && enabledCount > 0 && (
        <p className="mt-3 rounded-lg border border-warn/30 bg-warn-soft px-3 py-2.5 text-xs leading-relaxed text-warn">
          На iPhone сначала добавьте KaizenFlow на экран «Домой» через Safari —
          иначе системные уведомления недоступны.
        </p>
      )}

      {localPushReady && !remotePushReady && (
        <p className="mt-3 rounded-lg bg-success-soft px-3 py-2 text-xs leading-relaxed text-success">
          Локальные уведомления активны — сервер не нужен
        </p>
      )}

      {remotePushReady && permission === 'granted' && (
        <p className="mt-3 rounded-lg bg-success-soft px-3 py-2 text-xs leading-relaxed text-success">
          Фоновые push через сервер активны
        </p>
      )}
      ...
      <div className="mt-4 space-y-1 text-xs leading-relaxed text-ink-faint">
        <p className="m-0">Подсказки внутри приложения следуют этим настройкам.</p>
        <p className="m-0">
          На iOS уведомления приходят при открытии приложения (утро после 8:00,
          возврат после паузы). Без сервера и без фона, пока приложение закрыто.
        </p>
      </div>
